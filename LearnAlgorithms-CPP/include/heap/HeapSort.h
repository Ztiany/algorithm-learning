...
template <typename Item> void heapSortV1(Item *item, int size) {
...
/** 优化：拷贝 + heapify */
template <typename Item> void heapSortV2(Item *item, int size) {
...
template <typename Item> void heapSortV3(Item *item, int size) {
  ...
  for (int j = size - 1; j > 0; --j) {
    std::swap(item[j], item[0]);
    /* 每次都把最大值放到之后，把最后那个元素放到最前面除掉之后部分的以排序的最大值，把前面未排序的部分看做一个堆，堆根节点做 heapify 操作。*/
    shiftDown(item, j, 0);
  }
}
