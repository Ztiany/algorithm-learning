...
#ifndef MAX_INDEX_HEAP_V2_H
#define MAX_INDEX_HEAP_V2_H
 ...
 */
template <typename Item> class IndexMaxHeapV2 {

  Item *data;
  int capacity{};
  int count{};
  // indexes 用来维持 data 中元素在堆中的索引
  int *indexes; //  最大索引堆中的索引, indexes[x] = i 表示索引 i
                //  指向的数组中的值在堆中的位置是 x
  int *reverse; // 最大索引堆中的反向索引, reverse[i] = x 表示数组中索引 i
                // 指向的值，在 indexes 中的索引是 x
  ...
  void shiftUP(int index) {
    while (index > 1) {
      ...
      // 维护 reverse
      // 假设现在 indexes[index] = 3，那么 data[3] 的值在 indexes 中的索引就是
      // 3，那么 reverse[3] = index。
  ...
  void shiftDown(int index) {
    while (leftChild(index) <= count) {
      ...
      if (target + 1 <= count &&
          data[indexes[target + 1]] > data[indexes[target]]) {
        target++;
      }
...
public:
  ...
  void change(int index, Item newItem) {
    ...
    index += 1;
    data[index] = newItem;
    // 因为 data[index] = newItem = data[indexes[reverse[index]]]，所以 index =
    // indexes[reverse[index]]，则 index 在 indexes 中的位置就是 reverse[index]
