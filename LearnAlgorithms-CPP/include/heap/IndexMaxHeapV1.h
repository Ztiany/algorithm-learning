...
#ifndef MAX_INDEX_HEAP_V1_H
#define MAX_INDEX_HEAP_V1_H
 ...
 */
template <typename Item> class IndexMaxHeapV1 {
...
public:
  ...
  /* 如果返回引用，那么返回的将会是局部变量的引用，所以还是返回拷贝 */
  Item extractMax() {
    if (isEmpty()) {
  ...
  void change(int index, Item newItem) {
    ...
    // 之后 shiftUp(j), 再shiftDown(j)，这是为了维护堆的性质，因为change的这个数是无法确定的，所以两个操作都要试一下。
